...
object IndependentPrecompiledHandlerPackagedConsumerSpec {
  ...
  def run(repositoryRoot: File): Unit = {
    ...
    check(bodyViewHandler.contains("DirectTypeShape.NamedType"), "simple named type shape is not used")
    check(!bodyViewHandler.contains(".summary"), "body-view handler parses Unsupported.summary")
    ...
    check(bodyViewConsumer.contains("trait IndependentShow[A]"), "body-view consumer lacks representative Show trait")
    check(bodyViewConsumer.contains("def show(a: A): String"), "body-view consumer lacks the exact delegated inspection shape")
    check(bodyViewConsumer.contains("IndependentShow.independentBodyView"), "body-view consumer does not use generated companion output")
    ...
    check(bodyViewNegative.contains("def show(a: A): List[String]"), "body-view negative does not isolate the applied result type")
    check(bodyViewNegative.contains("List[String]"), "body-view negative lacks an unsupported applied type")
