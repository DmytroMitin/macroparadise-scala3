...
/** Experimental read-only syntactic view of an annotated class or trait target.
  *
  ...
  * unrestricted target AST continue to use `ExpansionInput.primary.tree`.
  * All fields and positions remain exact-compiler-sensitive experimental data.
  */
final case class ExpansionTargetView(
    className: String,
    typeParameters: List[ExpansionTargetView.TypeParameter],
    constructorClauses: List[ExpansionTargetView.ConstructorClause],
    modifiers: ExpansionTargetView.Modifiers,
    classPos: SrcPos,
    constructorPos: SrcPos,
    definitionKind: ExpansionTargetView.DefinitionKind = ExpansionTargetView.DefinitionKind.Class
):
  ...
  def this(
    ...
    classPos,
    constructorPos,
    ExpansionTargetView.DefinitionKind.Class
  )
...
object ExpansionTargetView:
  ...
  /** Decode one raw class tree without applying annotation admission policy.
    ...
    */
  def decode(tree: untpd.Tree | Null)(using Context): Either[ExpansionDiagnostic, ExpansionTargetView] =
    if tree == null then
  ...
  private def decodeTypeDef(typeDef: TypeDef)(using Context): Either[ExpansionDiagnostic, ExpansionTargetView] =
    ...
    if !typeDef.isClassDef then
      Left(failure(s"unsupported annotation target `$className`: expected a raw class or trait template", classPos))
    else if mods.is(Enum) then
      Left(failure(s"unsupported annotation target `enum $className`: expected a raw class or trait template", classPos))
    else
  ...
  )(using Context): Either[ExpansionDiagnostic, ExpansionTargetView] =
