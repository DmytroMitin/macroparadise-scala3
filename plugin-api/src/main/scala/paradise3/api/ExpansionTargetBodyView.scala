...
/** Experimental syntactic pre-typer read-only view of a class template's
  ...
  * `ExpansionInput.primary.tree` escape hatch.
  */
final case class ExpansionTargetBodyView(
    members: List[ExpansionTargetBodyView.DirectMember],
    pos: SrcPos
)

object ExpansionTargetBodyView:
  ...
  def decode(tree: untpd.Tree | Null)(using Context): Either[ExpansionDiagnostic, ExpansionTargetBodyView] =
    if tree == null then
  ...
  private def decodeTypeDef(typeDef: TypeDef)(using Context): Either[ExpansionDiagnostic, ExpansionTargetBodyView] =
  ...
  private def decodeTemplate(
  ...
  )(using Context): Either[ExpansionDiagnostic, ExpansionTargetBodyView] =
    ...
    requireList(template.body, "direct template-body member list", bodyPos).flatMap: body =>
      traverse(body)(normalizeMember(_, classView.typeParameters.map(_.name).toSet, bodyPos)).map: members =>
        ExpansionTargetBodyView(members, bodyPos)
