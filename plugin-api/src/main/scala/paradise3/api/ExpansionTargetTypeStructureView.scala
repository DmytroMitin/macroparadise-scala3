...
/** Experimental syntactic pre-typer view of enclosing type-parameter bounds
  ...
  * [[ExpansionTargetBodyView.DirectTypeShape]]; advanced exact-compiler
  * handlers retain the raw `ExpansionInput.primary.tree` escape hatch.
  */
final case class ExpansionTargetTypeStructureView(
    typeParameters: List[ExpansionTargetTypeStructureView.EnclosingTypeParameter],
    directTypeMembers: List[ExpansionTargetTypeStructureView.DirectTypeMember],
    pos: SrcPos
)

object ExpansionTargetTypeStructureView:
  import ExpansionTargetBodyView.{DirectTypeShape, DirectVisibility}
  ...
  final case class EnclosingTypeParameter(
      name: String,
      variance: ExpansionTargetView.Variance,
      lowerBound: Bound,
      upperBound: Bound,
  ...
  def decode(tree: untpd.Tree | Null)(using Context): Either[ExpansionDiagnostic, ExpansionTargetTypeStructureView] =
    if tree == null then
  ...
  private def decodeTypeDef(
      typeDef: TypeDef
  )(using Context): Either[ExpansionDiagnostic, ExpansionTargetTypeStructureView] =
    ExpansionTargetView.decode(typeDef).flatMap: classView =>
  ...
  private def decodeTemplate(
    ...
    Option(template.constr) match
      ...
      case Some(constructor) =>
        ...
        yield ExpansionTargetTypeStructureView(typeParameters, typeMembers, viewPos)
  ...
  private def normalizeEnclosingTypeParameter(
    ...
    Option(parameter) match
      ...
      case Some(value) =>
        ...
        val variance =
          if mods.is(Covariant) then ExpansionTargetView.Variance.Covariant
          else if mods.is(Contravariant) then ExpansionTargetView.Variance.Contravariant
          else ExpansionTargetView.Variance.Invariant
